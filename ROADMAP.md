...
| The piano roll's marker accessors are a silent no-op | measured 2026-09-19 on FL Studio 2026 build 5406: `flpianoroll.score.markerCount` and `getMarker(index)` exist, raise nothing, and report **zero markers** in a project whose arrangement holds three, one of them a time signature marker. The stub's `Marker` documents `mode` 8 and 12 with time signature and key fields, so the API is meant for playlist markers. The read returns an empty list with no error, which is the same silent failure mode as the unknown-action bug and the plugin argument order: the code runs, reports success, and does nothing. Marker times are therefore unreadable in both sandboxes, `arrangement.getMarkers` reports `time: null` because no `getMarkerTime` exists, and `fl_structure_critique` reports section lengths as unavailable rather than guessing them |
| `findFirstNextEmptyPat` freezes FL, whatever the flags | measured twice on 2026-09-19, each time costing the session: first as `findFirstNextEmptyPat(0)`, which per `midi/__ffnep_flags.py` also asks FL to prompt for a pattern name, and then as `FFNEP_FindFirst \| FFNEP_DontPromptName` with the documented `None` return no longer used. Both froze FL Studio 2026 build 5406: the window stopped responding, the call never returned, and every later MIDI command timed out. The prompt was therefore not the cause, and the function is unusable on this build. `fl_create_pattern` and the clone branch of `fl_set_pattern` now refuse and point at creating the pattern by hand and selecting it with `patterns.select`. The stub also declares the return `-> None` (`patterns/__properties.py:189`), which the old handler compared with an integer; `tests/test_none_returning_calls.py` now refuses any call in either script that consumes a value the stubs declare as None |
| A virtual port must exist before FL can be told about it | after restarting FL the "FL Studio MCP" input is absent from Options > MIDI Settings, because the port only exists while a process holds it open. FL can only offer it for selection while some process has created it, so the server has to be running (or the port held) before the producer enables it |
...
| Undo grouping | batches wrap in `general.saveUndo()` so one AI edit is one Ctrl+Z. **Not achievable, measured:** `general.saveUndo` adds no history entry and does not reduce how many undos an edit needs, and the API has no grouping flag. What batching does deliver is one trigger and all-or-nothing execution, stopping at the first failure. `fl_undo(steps=n)` moves relatively, because `general.undo()` is a toggle |
